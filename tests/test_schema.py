...
class TestTypeErrorsHandling(TestCase):
...
class TestValidationErrorsHandling(TestCase):
    def test_default_behavior_raise(self):
        schema = Schema.build(
            {
                "file_type": "csv",
...
class TestPostProcessorsErrorsHandling(TestCase):
...
class TestPreProcessorsErrorsHandling(TestCase):
