...
class TestStr(TestCase):
    def test_transform(self):
        type_converter = TypeConverter.build({"type": "str"})
        assert type_converter.transform("hello") == "hello"


class TestInt(TestCase):
    def test_transform(self):
        type_converter = TypeConverter.build({"type": "int"})
        assert type_converter.transform("153") == 153

    def test_transform_failed(self):
        type_converter = TypeConverter.build({"type": "int"})

        with pytest.raises(ValueError, match="value 'abc' is not a valid integer"):
            type_converter.transform("abc")


class TestDecimal(TestCase):
    def test_transform(self):
        type_converter = TypeConverter.build({"type": "decimal"})
        assert type_converter.transform("153.56") == Decimal("153.56")
...
class TestTime(TestCase):
    def test_transform(self):
        type_converter = TypeConverter.build({"type": "time"})
        assert type_converter.transform("10:12:03+03:00") == time(
    ...
    def test_transform_failed(self):
        ...
        with pytest.raises(ValueError):
            type_converter.transform("Invalid")

    def test_transform_naive_time(self):
        type_converter = TypeConverter.build({"type": "time"})

        with pytest.raises(ValueError):
            type_converter.transform("10:12:03")


class TestDateTime(TestCase):
    def test_transform(self):
        type_converter = TypeConverter.build({"type": "datetime"})
        assert type_converter.transform("2022-01-12T10:12:03+03:00") == datetime(
    ...
    def test_transform_naive_time(self):
        ...
        with pytest.raises(ValueError):
            type_converter.transform("2022-01-12T10:12:03")
...
class TestNullableField(TestCase):
    def test_int(self):
        ...
        assert type_converter.transform(None) is None
