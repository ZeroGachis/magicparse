...
class TestBuild(TestCase):
    def test_left_pad_zeroes(self):
        pre_processor = PreProcessor.build({"name": "left-pad-zeroes", "parameters": {"width": 10}})
    ...
    def test_left_strip(self):
        pre_processor = PreProcessor.build({"name": "left-strip", "parameters": {"characters": "0"}})
...
class TestReplace(TestCase):
    def test_pattern_not_found(self):
        pre_processor = PreProcessor.build({"name": "replace", "parameters": {"pattern": "bbb", "replacement": "BBB"}})
...
class TestRegexExtract(TestCase):
    def test_build_without_value_group(self):
        with pytest.raises(
            ...
            PreProcessor.build({"name": "regex-extract", "parameters": {"pattern": "xxx"}})
    ...
    def test_pattern_not_found(self):
        ...
        assert error.value.args[0] == "cannot extract value from pattern '^xxx(?P<value>\\d{13})xxx$'"
